...
const ChallengeDetailsPage: React.FC = () => {
  ...
  const handleAcceptChallenge = () => {
    ...
    // Save the challenge ID to localStorage
    const storedAcceptedChallenges = localStorage.getItem('acceptedChallenges');
    let acceptedChallengeIds: string[] = [];

    if (storedAcceptedChallenges) {
        try {
          acceptedChallengeIds = JSON.parse(storedAcceptedChallenges) as string[];
        } catch (error) {
          console.error("Error parsing acceptedChallenges from localStorage:", error);
          // Handle the error appropriately (e.g., clear localStorage or start with an empty array)
          acceptedChallengeIds = []; // Start with an empty array in case of parsing error
        }
    }

    if (!acceptedChallengeIds.includes(challengeId)) {
        acceptedChallengeIds.push(challengeId);
        localStorage.setItem('acceptedChallenges', JSON.stringify(acceptedChallengeIds));
        console.log("Challenge ID saved to localStorage:", challengeId); // Confirm it's saved
        console.log("localStorage after saving:", localStorage.getItem('acceptedChallenges'));
    }

    // After accepting, redirect to the challenge status page:
    router.push('/challenge-list');  // Adjust the route if needed
  };
